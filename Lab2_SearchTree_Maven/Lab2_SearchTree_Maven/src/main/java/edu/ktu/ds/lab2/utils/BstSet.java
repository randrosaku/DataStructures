...
public class BstSet<E extends Comparable<E>> implements SortedSet<E>, Cloneable {
    ...
    public E floor(E e) {
        ...
        BstNode<E> node = root;
        BstNode<E> parent = null;
        BstNode<E> first = root;
        while (node != null) {
            int cmp = c.compare(e, node.element);
            if(!(first.equals(node)) && cmp < 0) {
                break;
            }
            else if (cmp < 0) {
                parent = node;
    ...
    public E pollFirst() {
        ...
        if(root == null)
            return null;
        pollF = get(root, false).element;
        root = removeRecursive(get(root, false).element, root);
    ...
    public E pollLast() {
        ...
        if(root == null)
            return null;
        pollL = get(root, true).element;
        root = removeRecursive(get(root, true).element, root);

        return pollL;
    }

    public SortedSet<E> copyOf(BstSet<? extends E> set) {
        BstSet<E> set2 = (BstSet<E>) set;
        if(set == null || root == null){
            return null;
        }

        BstSet<E> setCopy = new BstSet<E>();
        setCopy.root = cloneRecursive(set2.root);
        setCopy.size = set2.size;
    ...
    protected class BstNode<N> {
        ...
        // Element
        protected N element;
        // Pointer to the left subtree x <
        protected BstNode<N> left;
        // Pointer to the right subtree x >
