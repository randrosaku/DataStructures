...
public class HashMapOa<K, V> implements EvaluableMap<K, V> {
    ...
    public V remove(K key) {
        ...
        if(table[index] == DELETED){ return null; }

        V deleteValue = table[index].value;
        table[index] = DELETED;
    ...
    public boolean replace(K key, V oldValue, V newValue) {
        if (key == null) {
            throw new IllegalArgumentException("Key is null in replace method");
        }
    ...
    public boolean containsValue(Object value) {
        return Arrays.stream(table)
                .filter(entry -> entry != null && !DELETED.equals(entry) && entry.value == value).toArray().length >= 1;
    }
