...
public class HashMap<K, V> implements EvaluableMap<K, V> {
    ...
    public int averageChainSize() {
        int averageSize = -1;
        int counter = 0;
        int buckets = table.length - emptyElements();
    ...
    public int averageChainSize() {
        ...
        averageSize = size / (table.length - emptyElements());
        return averageSize;
    }
    ...
    public K putIfAbsent(K key, V value) {
        for (Node<K, V> n : table) {
            for(Node<K, V> nn = n; nn != null; nn = nn.next) {
                if (nn.value.equals(value) && nn.key.equals(key)){
                    return key;
                }
            }
        }
